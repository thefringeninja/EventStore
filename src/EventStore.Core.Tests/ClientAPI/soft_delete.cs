...
namespace EventStore.Core.Tests.ClientAPI {
	...
	public class soft_delete : IClassFixture<soft_delete.Fixture> { public class Fixture : SpecificationWithDirectoryPerTestFixture {
		...
		public async Task soft_deleted_stream_allows_recreation_when_expver_any() {
			...
			var res = await _conn.ReadStreamEventsForwardAsync(stream, 0, 100, false);
			Assert.Equal(new[] {2L, 3L, 4L}, res.Events.Select(x => x.OriginalEvent.EventNumber));
		...
		public async Task soft_deleted_stream_allows_recreation_when_expver_is_exact() {
			...
			var events = new[] {TestEvent.NewTestEvent(), TestEvent.NewTestEvent(), TestEvent.NewTestEvent()};
			Assert.Equal(4, (await _conn.AppendToStreamAsync(stream, 1, events)).NextExpectedVersion);
		...
		public async Task soft_deleted_stream_when_recreated_preserves_metadata_except_truncatebefore() {
			...
			Assert.Equal(0, (await _conn.SetStreamMetadataAsync(stream, ExpectedVersion.NoStream,
			...
			Assert.True(meta.StreamMetadata.GetValue<bool>("key1"));
		...
		public async Task soft_deleted_stream_appends_both_writes_when_expver_any() {
			...
			Assert.Equal(events1.Concat(events2).Select(x => x.EventId),
				res.Events.Select(x => x.OriginalEvent.EventId));
			Assert.Equal(new[] {2L, 3L, 4L, 5L, 6L}, res.Events.Select(x => x.OriginalEvent.EventNumber));
		...
		public async Task
			...
			Assert.Equal(1, (await _conn.SetStreamMetadataAsync(stream, 0,
		...
		public async Task
			...
			Assert.Equal(1, (await _conn.SetStreamMetadataAsync(stream, 0, new byte[256])).NextExpectedVersion);
		...
		public async Task
			...
			var res = await _conn.ReadStreamEventsForwardAsync(stream, 0, 100, false);
			Assert.Equal(new[] {0L, 1L}, res.Events.Select(x => x.OriginalEventNumber).ToArray());
			Assert.Equal(SliceReadStatus.Success, res.Status);
			Assert.Equal(1, res.LastEventNumber);
