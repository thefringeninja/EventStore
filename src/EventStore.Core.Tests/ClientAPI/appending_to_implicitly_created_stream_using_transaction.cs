		...
		*/
		...
		public async Task sequence_0em1_1e0_2e1_3e2_4e3_5e4_0em1_idempotent() {
			...
			using (var store = BuildConnection(_node)) {
				...
				Assert.Equal(5, (await (await (await writer.StartTransaction(-1)).Write(events)).Commit()).NextExpectedVersion);
				Assert.Equal(0, (await (await (await writer.StartTransaction(-1)).Write(events.First())).Commit()).NextExpectedVersion);
		...
		public async Task sequence_0em1_1e0_2e1_3e2_4e3_5e4_0any_idempotent() {
			...
			using (var store = BuildConnection(_node)) {
				...
				Assert.Equal(5, (await (await (await writer.StartTransaction(-1)).Write(events)).Commit()).NextExpectedVersion);
				Assert.Equal(0, (await (await (await writer.StartTransaction(ExpectedVersion.Any)).Write(events.First())).Commit()).NextExpectedVersion);
		...
		public async Task sequence_0em1_1e0_2e1_3e2_4e3_5e4_0e5_non_idempotent() {
			...
			using (var store = BuildConnection(_node)) {
				...
				Assert.Equal(5, (await (await (await writer.StartTransaction(-1)).Write(events)).Commit()).NextExpectedVersion);
				Assert.Equal(6, (await (await (await writer.StartTransaction(5)).Write(events.First())).Commit()).NextExpectedVersion);
		...
		public async Task sequence_0em1_0e0_non_idempotent() {
			...
			using (var store = BuildConnection(_node)) {
				...
				Assert.Equal(0, (await (await (await writer.StartTransaction(-1)).Write(events)).Commit()).NextExpectedVersion);
				Assert.Equal(1, (await (await (await writer.StartTransaction(0)).Write(events.First())).Commit()).NextExpectedVersion);
		...
		public async Task sequence_0em1_1e0_2e1_1any_1any_idempotent() {
			...
			using (var store = BuildConnection(_node)) {
				...
				Assert.Equal(2, (await (await (await writer.StartTransaction(-1)).Write(events)).Commit()).NextExpectedVersion);
				Assert.Equal(1, (await (await (await (await writer.StartTransaction(ExpectedVersion.Any)).Write(events[1])).Write(events[1])).Commit()).NextExpectedVersion);
		...
		public async Task sequence_S_0em1_1em1_E_S_0em1_1em1_2em1_E_idempotancy_fail() {
			...
			using (var store = BuildConnection(_node)) {
				...
				Assert.Equal(1, (await (await (await writer.StartTransaction(-1)).Write(events)).Commit()).NextExpectedVersion);
				Assert.ThrowsAsync<WrongExpectedVersionException>(async () =>
