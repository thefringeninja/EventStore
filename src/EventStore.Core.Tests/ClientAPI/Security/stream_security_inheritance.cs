...
namespace EventStore.Core.Tests.ClientAPI.Security {
	...
	public class stream_security_inheritance : AuthenticationTestBase {
		...
		public async Task acl_inheritance_is_working_properly_on_user_streams() {
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-no-acl", null, null));
			await WriteStream("user-no-acl", "user1", "pa$$1");
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-no-acl", "user2", "pa$$2"));
			await WriteStream("user-no-acl", "adm", "admpa$$");

			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-w-diff", null, null));
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-w-diff", "user1", "pa$$1"));
			await WriteStream("user-w-diff", "user2", "pa$$2");
			await WriteStream("user-w-diff", "adm", "admpa$$");

			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-w-multiple", null, null));
			...
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-w-restricted", null, null));
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-w-restricted", "user1", "pa$$1"));
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("user-w-restricted", "user2", "pa$$2"));
			...
			await Assert.ThrowsAsync<AccessDeniedException>(() => ReadEvent("user-r-restricted", null, null));
			await ReadEvent("user-r-restricted", "user1", "pa$$1");
			await Assert.ThrowsAsync<AccessDeniedException>(() => ReadEvent("user-r-restricted", "user2", "pa$$2"));
			await ReadEvent("user-r-restricted", "adm", "admpa$$");
		}

		[Fact]
		public async Task acl_inheritance_is_working_properly_on_system_streams() {
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-no-acl", null, null));
			await WriteStream("$sys-no-acl", "user1", "pa$$1");
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-no-acl", "user2", "pa$$2"));
			await WriteStream("$sys-no-acl", "adm", "admpa$$");

			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-w-diff", null, null));
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-w-diff", "user1", "pa$$1"));
			await WriteStream("$sys-w-diff", "user2", "pa$$2");
			await WriteStream("$sys-w-diff", "adm", "admpa$$");

			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-w-multiple", null, null));
			...
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-w-restricted", null, null));
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-w-restricted", "user1", "pa$$1"));
			await Assert.ThrowsAsync<AccessDeniedException>(() => WriteStream("$sys-w-restricted", "user2", "pa$$2"));
			...
			await Assert.ThrowsAsync<AccessDeniedException>(() => ReadEvent("$sys-no-acl", null, null));
			await Assert.ThrowsAsync<AccessDeniedException>(() => ReadEvent("$sys-no-acl", "user1", "pa$$1"));
			await Assert.ThrowsAsync<AccessDeniedException>(() => ReadEvent("$sys-no-acl", "user2", "pa$$2"));
