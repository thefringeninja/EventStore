using System;
using System.Collections.Generic;
using EventStore.Core.Index;
using Xunit;
using EventStore.Core.Index.Hashes;
using System.IO;
using System.Threading.Tasks;

namespace EventStore.Core.Tests.Index.IndexV4 {
	public class when_merging_ptables_vx_to_v4 {
		public static IEnumerable<object[]> TestCases() {
			yield return new object[] {PTableVersions.IndexV1, false};
			yield return new object[] {PTableVersions.IndexV1, true};
			yield return new object[] {PTableVersions.IndexV2, false};
			yield return new object[] {PTableVersions.IndexV2, true};
			yield return new object[] {PTableVersions.IndexV3, false};
			yield return new object[] {PTableVersions.IndexV3, true};
			yield return new object[] {PTableVersions.IndexV4, false};
			yield return new object[] {PTableVersions.IndexV4, true};
		}

		[Theory, MemberData(nameof(TestCases))]
		public void merged_ptable_is_64bit(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(PTableVersions.IndexV4, fixture.NewTable.Version);
		}

		[Theory, MemberData(nameof(TestCases))]
		public void there_are_8_records_in_the_merged_index(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(8, fixture.NewTable.Count);
		}

		[Theory, MemberData(nameof(TestCases))]
		public void midpoints_are_cached_in_ptable_footer(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			var numIndexEntries = 8;
			var requiredMidpoints = PTable.GetRequiredMidpointCountCached(numIndexEntries, PTableVersions.IndexV4);

			var newTableFileCopy = fixture.GetTempFilePath();
			File.Copy(fixture.NewTableFile, newTableFileCopy);
			using (var filestream = File.Open(newTableFileCopy, FileMode.Open, FileAccess.Read)) {
				var footerSize = PTableFooter.GetSize(PTableVersions.IndexV4);
				Assert.Equal(filestream.Length,
					PTableHeader.Size + numIndexEntries * PTable.IndexEntryV4Size +
					requiredMidpoints * PTable.IndexEntryV4Size + footerSize + PTable.MD5Size);
				filestream.Seek(
					PTableHeader.Size + numIndexEntries * PTable.IndexEntryV4Size +
					requiredMidpoints * PTable.IndexEntryV4Size, SeekOrigin.Begin);

				var ptableFooter = PTableFooter.FromStream(filestream);
				Assert.Equal(FileType.PTableFile, ptableFooter.FileType);
				Assert.Equal(PTableVersions.IndexV4, ptableFooter.Version);
				Assert.Equal(requiredMidpoints, ptableFooter.NumMidpointsCached);
			}
		}

		[Theory, MemberData(nameof(TestCases))]
		public void correct_number_of_midpoints_are_loaded(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(fixture.NewTable.GetMidPoints().LongLength,
				PTable.GetRequiredMidpointCountCached(8, PTableVersions.IndexV4));
		}

		[Theory, MemberData(nameof(TestCases))]
		public void all_the_entries_have_upgraded_hashes(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			if (fixture.FromVersion == PTableVersions.IndexV1) {
				foreach (var item in fixture.NewTable.IterateAllInOrder()) {
					Assert.True((ulong)item.Position == item.Stream - 1);
				}
			}
		}

		[Theory, MemberData(nameof(TestCases))]
		public void none_of_the_entries_have_upgraded_hashes(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			if (fixture.FromVersion > PTableVersions.IndexV1) {
				foreach (var item in fixture.NewTable.IterateAllInOrder()) {
					Assert.True((ulong)item.Position == item.Stream);
				}
			}
		}

		class Fixture : DirectoryFixture {
			private readonly List<string> _files = new List<string>();
			private readonly List<PTable> _tables = new List<PTable>();

			public readonly PTable NewTable;
			public readonly string NewTableFile;
			public readonly byte FromVersion;

			public Fixture(byte fromVersion, bool skipIndexVerify) {
				FromVersion = fromVersion;
				_files.Add(GetTempFilePath());
				var table = new HashListMemTable(FromVersion, maxSize: 20);
				if (FromVersion == PTableVersions.IndexV1) {
					table.Add(0x010100000000, 0, 0x0101);
					table.Add(0x010200000000, 0, 0x0102);
					table.Add(0x010300000000, 0, 0x0103);
					table.Add(0x010400000000, 0, 0x0104);
				} else {
					table.Add(0x0101, 0, 0x0101);
					table.Add(0x0102, 0, 0x0102);
					table.Add(0x0103, 0, 0x0103);
					table.Add(0x0104, 0, 0x0104);
				}

				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				table = new HashListMemTable(FromVersion, maxSize: 20);

				if (FromVersion == PTableVersions.IndexV1) {
					table.Add(0x010500000000, 0, 0x0105);
					table.Add(0x010600000000, 0, 0x0106);
					table.Add(0x010700000000, 0, 0x0107);
					table.Add(0x010800000000, 0, 0x0108);
				} else {
					table.Add(0x0105, 0, 0x0105);
					table.Add(0x0106, 0, 0x0106);
					table.Add(0x0107, 0, 0x0107);
					table.Add(0x0108, 0, 0x0108);
				}

				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				NewTableFile = GetTempFilePath();
				NewTable = PTable.MergeTo(_tables, NewTableFile, (streamId, hash) => hash + 1, x => true,
					x => new Tuple<string, bool>(x.Stream.ToString(), true), PTableVersions.IndexV4,
					skipIndexVerify: skipIndexVerify);
			}

			public override void Dispose() {
				NewTable.Dispose();
				foreach (var ssTable in _tables) {
					ssTable.Dispose();
				}
				
				base.Dispose();
			}
		}
	}

	public class when_merging_to_ptable_v4_with_deleted_entries_from_v1 {
		public static IEnumerable<object[]> TestCases() {
			yield return new object[] {PTableVersions.IndexV1, false};
			yield return new object[] {PTableVersions.IndexV1, true};
		}

		[Theory, MemberData(nameof(TestCases))]
		public void merged_ptable_is_64bit(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(PTableVersions.IndexV4, fixture.NewTable.Version);
		}

		[Theory, MemberData(nameof(TestCases))]
		public void there_are_7_records_in_the_merged_index(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(7, fixture.NewTable.Count);
		}

		[Theory, MemberData(nameof(TestCases))]
		public void midpoints_are_cached_in_ptable_footer(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			var numIndexEntries = 7;
			var requiredMidpoints = PTable.GetRequiredMidpointCountCached(numIndexEntries, PTableVersions.IndexV4);

			var newTableFileCopy = fixture.GetTempFilePath();
			File.Copy(fixture.NewTableFile, newTableFileCopy);
			using (var filestream = File.Open(newTableFileCopy, FileMode.Open, FileAccess.Read)) {
				var footerSize = PTableFooter.GetSize(PTableVersions.IndexV4);
				Assert.Equal(filestream.Length,
					PTableHeader.Size + numIndexEntries * PTable.IndexEntryV4Size +
					requiredMidpoints * PTable.IndexEntryV4Size + footerSize + PTable.MD5Size);
				filestream.Seek(
					PTableHeader.Size + numIndexEntries * PTable.IndexEntryV4Size +
					requiredMidpoints * PTable.IndexEntryV4Size, SeekOrigin.Begin);

				var ptableFooter = PTableFooter.FromStream(filestream);
				Assert.Equal(FileType.PTableFile, ptableFooter.FileType);
				Assert.Equal(PTableVersions.IndexV4, ptableFooter.Version);
				Assert.Equal(requiredMidpoints, ptableFooter.NumMidpointsCached);
			}
		}

		[Theory, MemberData(nameof(TestCases))]
		public void correct_number_of_midpoints_are_loaded(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(fixture.NewTable.GetMidPoints().LongLength,
				PTable.GetRequiredMidpointCountCached(7, PTableVersions.IndexV4));
		}

		[Theory, MemberData(nameof(TestCases))]
		public void the_items_are_sorted(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			var last = new IndexEntry(ulong.MaxValue, 0, long.MaxValue);
			foreach (var item in fixture.NewTable.IterateAllInOrder()) {
				Assert.True((last.Stream == item.Stream ? last.Version > item.Version : last.Stream > item.Stream) ||
				            ((last.Stream == item.Stream && last.Version == item.Version) &&
				             last.Position > item.Position));
				last = item;
			}
		}

		class Fixture : DirectoryFixture {
			private readonly List<string> _files = new List<string>();
			private readonly List<PTable> _tables = new List<PTable>();
			public readonly string NewTableFile;
			public readonly PTable NewTable;

			public Fixture(byte fromVersion, bool skipIndexVerify) {
				IHasher hasher = new Murmur3AUnsafe();
				_files.Add(GetTempFilePath());
				var table = new HashListMemTable(fromVersion, maxSize: 20);
				table.Add(0x010100000000, 0, 1);
				table.Add(0x010200000000, 0, 2);
				table.Add(0x010300000000, 0, 3);
				table.Add(0x010300000000, 1, 4);
				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				table = new HashListMemTable(fromVersion, maxSize: 20);
				table.Add(0x010100000000, 2, 5);
				table.Add(0x010200000000, 1, 6);
				table.Add(0x010200000000, 2, 7);
				table.Add(0x010400000000, 0, 8);
				table.Add(0x010400000000, 1, 9);
				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				table = new HashListMemTable(fromVersion, maxSize: 20);
				table.Add(0x010100000000, 1, 10);
				table.Add(0x010100000000, 2, 11);
				table.Add(0x010500000000, 1, 12);
				table.Add(0x010500000000, 2, 13);
				table.Add(0x010500000000, 3, 14);
				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				NewTableFile = GetTempFilePath();
				NewTable = PTable.MergeTo(_tables, NewTableFile,
					(streamId, hash) => hash << 32 | hasher.Hash(streamId),
					x => x.Position % 2 == 0, x => new Tuple<string, bool>(x.Stream.ToString(), x.Position % 2 == 0),
					PTableVersions.IndexV4, skipIndexVerify: skipIndexVerify);
			}

			public override void Dispose() {
				NewTable.Dispose();
				foreach (var ssTable in _tables) {
					ssTable.Dispose();
				}
				
				base.Dispose();
			}
		}
	}

	public class when_merging_to_ptable_v4_with_deleted_entries {
		public static IEnumerable<object[]> TestCases() {
			yield return new object[] {PTableVersions.IndexV2, false};
			yield return new object[] {PTableVersions.IndexV2, true};
			yield return new object[] {PTableVersions.IndexV3, false};
			yield return new object[] {PTableVersions.IndexV3, true};
			yield return new object[] {PTableVersions.IndexV4, false};
			yield return new object[] {PTableVersions.IndexV4, true};
		}

		[Theory, MemberData(nameof(TestCases))]
		public void merged_ptable_is_64bit(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(PTableVersions.IndexV4, fixture.NewTable.Version);
		}

		[Theory, MemberData(nameof(TestCases))]
		public void there_are_14_records_in_the_merged_index(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(14, fixture.NewTable.Count);
		}

		[Theory, MemberData(nameof(TestCases))]
		public void midpoints_are_cached_in_ptable_footer(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			var numIndexEntries = 14;
			var requiredMidpoints = PTable.GetRequiredMidpointCountCached(numIndexEntries, PTableVersions.IndexV4);

			var newTableFileCopy = fixture.GetTempFilePath();
			File.Copy(fixture.NewTableFile, newTableFileCopy);
			using (var filestream = File.Open(newTableFileCopy, FileMode.Open, FileAccess.Read)) {
				var footerSize = PTableFooter.GetSize(PTableVersions.IndexV4);
				Assert.Equal(filestream.Length,
					PTableHeader.Size + numIndexEntries * PTable.IndexEntryV4Size +
					requiredMidpoints * PTable.IndexEntryV4Size + footerSize + PTable.MD5Size);
				filestream.Seek(
					PTableHeader.Size + numIndexEntries * PTable.IndexEntryV4Size +
					requiredMidpoints * PTable.IndexEntryV4Size, SeekOrigin.Begin);

				var ptableFooter = PTableFooter.FromStream(filestream);
				Assert.Equal(FileType.PTableFile, ptableFooter.FileType);
				Assert.Equal(PTableVersions.IndexV4, ptableFooter.Version);
				Assert.Equal(requiredMidpoints, ptableFooter.NumMidpointsCached);
			}
		}

		[Theory, MemberData(nameof(TestCases))]
		public void correct_number_of_midpoints_are_loade(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			Assert.Equal(fixture.NewTable.GetMidPoints().LongLength,
				PTable.GetRequiredMidpointCountCached(14, PTableVersions.IndexV4));
		}

		[Theory, MemberData(nameof(TestCases))]
		public void the_items_are_sorted(byte fromVersion, bool skipIndexVerify) {
			using var fixture = new Fixture(fromVersion, skipIndexVerify);
			var last = new IndexEntry(ulong.MaxValue, 0, long.MaxValue);
			foreach (var item in fixture.NewTable.IterateAllInOrder()) {
				Assert.True((last.Stream == item.Stream ? last.Version > item.Version : last.Stream > item.Stream) ||
				            ((last.Stream == item.Stream && last.Version == item.Version) &&
				             last.Position > item.Position));
				last = item;
			}
		}

		class Fixture : DirectoryFixture {
			private readonly List<string> _files = new List<string>();
			private readonly List<PTable> _tables = new List<PTable>();
			public readonly string NewTableFile;

			public readonly PTable NewTable;

			public Fixture(byte fromVersion, bool skipIndexVerify) {
				IHasher hasher = new Murmur3AUnsafe();
				_files.Add(GetTempFilePath());
				var table = new HashListMemTable(fromVersion, maxSize: 20);
				table.Add(0x010100000000, 0, 1);
				table.Add(0x010200000000, 0, 2);
				table.Add(0x010300000000, 0, 3);
				table.Add(0x010300000000, 1, 4);
				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				table = new HashListMemTable(fromVersion, maxSize: 20);
				table.Add(0x010100000000, 2, 5);
				table.Add(0x010200000000, 1, 6);
				table.Add(0x010200000000, 2, 7);
				table.Add(0x010400000000, 0, 8);
				table.Add(0x010400000000, 1, 9);
				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				table = new HashListMemTable(fromVersion, maxSize: 20);
				table.Add(0x010100000000, 1, 10);
				table.Add(0x010100000000, 2, 11);
				table.Add(0x010500000000, 1, 12);
				table.Add(0x010500000000, 2, 13);
				table.Add(0x010500000000, 3, 14);
				_tables.Add(PTable.FromMemtable(table, GetTempFilePath(), skipIndexVerify: skipIndexVerify));
				NewTableFile = GetTempFilePath();
				NewTable = PTable.MergeTo(_tables, NewTableFile,
					(streamId, hash) => hash << 32 | hasher.Hash(streamId),
					x => x.Position % 2 == 0, x => new Tuple<string, bool>(x.Stream.ToString(), x.Position % 2 == 0),
					PTableVersions.IndexV4, skipIndexVerify: skipIndexVerify);
			}

			public override void Dispose() {
				NewTable.Dispose();
				foreach (var ssTable in _tables) {
					ssTable.Dispose();
				}
				
				base.Dispose();
			}
		}
	}
}
