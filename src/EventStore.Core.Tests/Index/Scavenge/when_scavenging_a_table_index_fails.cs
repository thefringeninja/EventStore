...
namespace EventStore.Core.Tests.Index.Scavenge {
	public class when_scavenging_a_table_index_fails : SpecificationWithDirectoryPerTestFixture {
		...
		public override async Task TestFixtureSetUp() {
			...
			var ex = Assert.Throws<Exception>(() => _tableIndex.Scavenge(_log, CancellationToken.None));
		...
		public void should_have_logged_a_failure() {
			...
			Assert.False(_log.ScavengedIndices[0].Scavenged);
			Assert.Equal(_log.ScavengedIndices[0].Error, "Expected exception");
			Assert.Equal(_log.ScavengedIndices[0].EntriesDeleted, 0);
		}
