...
/*
        ...
        public void a_read_on_scavenged_chunk_includes_map()
        {
            ...
            using (var reader = chunk.AcquireReader())
            {
                ...
                var result = reader.ReadNextRawBytes(1024, buffer);
                Assert.False(result.IsEOF);
                Assert.Equal(ChunkHeader.Size + ChunkHeader.Size + 2 * PosMap.FullSize, result.BytesRead);
            }
            chunk.MarkForDeletion();
            chunk.WaitForDestroy(5000);
        }
        ...
        public void a_read_past_end_of_completed_chunk_does_include_header_or_footer()
        {
            ...
            using (var reader = chunk.AcquireReader())
            {
                ...
                Assert.Equal(ChunkHeader.Size + ChunkFooter.Size, result.BytesRead); //just header + footer = 256
            }
            chunk.MarkForDeletion();
            chunk.WaitForDestroy(5000);
        }
*/
		...
		public void a_read_past_eof_returns_eof_and_no_footer() {
			...
			using (var reader = chunk.AcquireReader()) {
				...
				Assert.True(result.IsEOF);
				Assert.Equal(4096, result.BytesRead); //does not includes header and footer space
			}
