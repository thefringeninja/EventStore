...
namespace EventStore.Core.Tests.Services.Storage.AllReader {
	public class when_a_single_write_before_the_transaction_is_present : RepeatableDbTestScenario {
		...
		public void should_be_able_to_read_the_transactional_writes_when_the_commit_is_present() {
			...
			Assert.Equal(4, firstRead.Records.Count);
			Assert.Equal("single_write_stream_id_1", firstRead.Records[0].Event.EventStreamId);
			Assert.Equal("single_write_stream_id_2", firstRead.Records[1].Event.EventStreamId);
			Assert.Equal("single_write_stream_id_3", firstRead.Records[2].Event.EventStreamId);
			Assert.Equal("single_write_stream_id_4", firstRead.Records[3].Event.EventStreamId);
