...
namespace EventStore.Core.Tests.Services.Storage.HashCollisions {
	public class
		with_three_collisioned_streams_one_event_each_with_second_stream_deleted_read_index_should :
		...
		public void return_correct_last_event_version_for_second_stream() {
			Assert.Equal(EventNumber.DeletedStream, ReadIndex.GetStreamLastEventNumber("CD"));
		}
		...
		public void return_all_events_on_read_all_forward() {
			...
			Assert.Equal(_prepare2, events[1]);
			Assert.Equal(_delete2, events[2]);
			Assert.Equal(_prepare3, events[3]);
		}
		...
		public void return_all_events_on_read_all_backward() {
			var events = ReadIndex.ReadAllEventsBackward(GetBackwardReadPos(), 100).Records.Select(r => r.Event)
				.ToArray();
			Assert.Equal(4, events.Length);
			Assert.Equal(_prepare1, events[3]);
			Assert.Equal(_prepare2, events[2]);
			Assert.Equal(_delete2, events[1]);
			Assert.Equal(_prepare3, events[0]);
		}
	}
}
