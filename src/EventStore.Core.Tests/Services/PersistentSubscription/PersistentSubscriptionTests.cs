...
namespace EventStore.Core.Tests.Services.PersistentSubscription {
	public class when_creating_persistent_subscription {
		...
		public when_creating_persistent_subscription() {
	...
	public class LiveTests {
		...
		public async Task
			...
			await Task.Delay(500).ContinueWith((action) => {
				...
				Assert.Equal(3, envelope.Replies.Count);
			});
		}
	}
	...
	public class Checkpointing {
		...
		public void
			...
			Assert.Equal(sub.OutstandingMessageCount, 2);
			//retry queue should be empty
			Assert.Equal(sub.StreamBuffer.RetryBufferCount, 0);
			...
			Assert.Equal(sub.OutstandingMessageCount, 0);
			Assert.Equal(sub.StreamBuffer.RetryBufferCount, 2);
	...
	public class NAKTests {
		...
		public void explicit_nak_with_park_correctly_tracks_the_available_client_slots() {
			...
			sub.NotAcknowledgeMessagesProcessed(corrid, new[] {id1}, NakAction.Park, "a reason from client.");
			Assert.Equal(2, envelope1.Replies.Count);
			Assert.True(parker.ParkedEvents.Any(_ => _.Event.EventId == id1));

			sub.NotAcknowledgeMessagesProcessed(corrid, new[] {id2}, NakAction.Park, "a reason from client.");
			Assert.True(parker.ParkedEvents.Any(_ => _.Event.EventId == id2));
