...
namespace EventStore.Core.Tests.Services.Transport.Http {
	public class build_requested_url_should {
		...
		public void with_advertise_http_port_set_only_port_is_changed() {
			...
			Assert.Equal(new Uri("http://www.example.com:2116/path/?key=value#anchor"), requestedUri);
		}
		...
		public void with_advertise_ip_set_only_host_is_changed() {
			...
			Assert.Equal(new Uri("http://127.0.1.13:1234/path/?key=value#anchor"), requestedUri);
		}
		...
		public void with_advertise_ip_and_http_port_set_both_host_and_port_is_changed() {
			...
			Assert.Equal(new Uri("http://127.0.1.13:2116/path/?key=value#anchor"), requestedUri);
		}
		...
		public void with_proto_forward_host_only_host_is_changed() {
			...
			var requestedUri =
				HttpEntity.BuildRequestedUrl(new FakeHttpRequest(inputUri, headers), null, 0);
			Assert.Equal(new Uri("http://www.my-host.com:1234/path/?key=value#anchor"), requestedUri);
		}

		[Fact, Trait("Category", "prefix")]
		public void with_forward_prefix_adds_prefix_when_already_prefix() {
			...
			var requestedUri =
				HttpEntity.BuildRequestedUrl(new FakeHttpRequest(inputUri, headers), null, 0);
			Assert.Equal(new Uri("http://www.example.com:1234/prefix/path/?key=value#anchor"), requestedUri);
		}
		...
		public void with_forward_prefix_adds_prefix_when_not_prefix() {
			...
			Assert.Equal(new Uri("http://www.my-host.com:1234/prefix/?key=value#anchor"), requestedUri);
		}
		...
		public void with_proto_forward_host_containing_comma_delimited_list_first_forwarded_host_is_used() {
			...
			var requestedUri =
				HttpEntity.BuildRequestedUrl(new FakeHttpRequest(inputUri, headers), null, 0);
			Assert.Equal(new Uri("http://www.my-first-host.com:1234/path/?key=value#anchor"), requestedUri);
		}
		...
		public void with_proto_forward_host_containing_hosts_with_ports_host_and_port_is_used() {
			...
			var requestedUri =
				HttpEntity.BuildRequestedUrl(new FakeHttpRequest(inputUri, headers), null, 0);
			Assert.Equal(new Uri("http://127.0.10.13:2231/path/?key=value#anchor"), requestedUri);
		}
