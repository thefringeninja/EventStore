...
namespace EventStore.Projections.Core.Tests.Services.event_reader.multi_stream_reader {
	public class when_handling_eof_for_all_streams_and_idle_eof : TestFixtureWithExistingEvents {
		...
		public void publishes_event_distribution_idle_messages() {
			Assert.Equal(2, Consumer.HandledMessages.OfType<ReaderSubscriptionMessage.EventReaderIdle>().Count());
			var first = Consumer.HandledMessages.OfType<ReaderSubscriptionMessage.EventReaderIdle>().First();
			var second = Consumer.HandledMessages.OfType<ReaderSubscriptionMessage.EventReaderIdle>().Skip(1).First();
