	...
	public class with_multiple_tracked_streams : SpecificationWithEmittedStreamsTrackerAndDeleter {
		...
		protected override async Task Given() {
			...
			for (int i = 0; i < _numberOfTrackedEvents; i++) {
				await Connection.AppendToStreamAsync(String.Format(_testStreamFormat, i), ExpectedVersion.Any,
			...
			if (!_eventAppeared.Wait(TimeSpan.FromSeconds(10))) {
				throw new Exception("Timed out waiting for emitted streams");
			}

			var emittedStreamResult =
                await Connection.ReadStreamEventsForwardAsync(_projectionNamesBuilder.GetEmittedStreamsName(), 0,
					_numberOfTrackedEvents, false, _credentials);
			Assert.Equal(_numberOfTrackedEvents, emittedStreamResult.Events.Length);
		...
		public async Task should_have_deleted_the_tracked_emitted_streams() {
			for (int i = 0; i < _numberOfTrackedEvents; i++) {
				var result = await Connection.ReadStreamEventsForwardAsync(String.Format(_testStreamFormat, i), 0, 1, false,
