...
namespace EventStore.Projections.Core.Tests.Integration.from_streams_matching {
	public class when_running_with_stream_metadata : specification_with_a_v8_query_posted {
		protected override void GivenEvents() {
			...
			ExistingStreamMetadata("stream1", "{\"include\":1}");
			ExistingStreamMetadata("stream2", "{\"include\":0}");
			ExistingStreamMetadata("stream3", "{\"include\":1}");
		}
...
fromStreamsMatching(function(s, streamMeta){
        return streamMeta.streamMetadata.include === 1;
}).when({
    $any: function(s, e) {
